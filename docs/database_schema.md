...
|----------|------|---------------|
| **PostgreSQL** | Primary metadata store | ACID compliance, strong consistency (critical for backup metadata), JSONB for flexible policy config, row-level security for multi-tenancy, read replicas for scalability |
